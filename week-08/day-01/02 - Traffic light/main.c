...
int main(void)
{
    ...
    __HAL_RCC_GPIOF_CLK_ENABLE(); //A pinek, hogy tudjanak villogni!
    ...
    LEDSF.Pin = GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10;
    LEDSF.Mode = GPIO_MODE_OUTPUT_PP;
    LEDSF.Pull = GPIO_NOPULL;
    LEDSF.Speed = GPIO_SPEED_HIGH;
    ...
    HAL_GPIO_Init(GPIOF, &LEDSF);
        while (1) {

            /* blinking the user LED with 1 Hz (1 on and 1 off per seconds) */
            tim_val = __HAL_TIM_GET_COUNTER(&TimHandle);

            HAL_GPIO_WritePin(GPIOF, GPIO_PIN_10, tim_val < 8000);

            HAL_GPIO_WritePin(GPIOF, GPIO_PIN_9, ((tim_val < 12000 && tim_val > 8000) || (tim_val > 20000 && tim_val < 24000)));


            HAL_GPIO_WritePin(GPIOF, GPIO_PIN_8, (tim_val < 24000 && tim_val > 12000));
        }
    }
