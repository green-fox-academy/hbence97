...
typedef enum {
	ICE_COLD, COLD, LUKEWARM, WARM, HOT
} temperature_t;
...
TIM_HandleTypeDef blue_led_pwm_handle;
TIM_HandleTypeDef red_led_pwm_handle;

UART_HandleTypeDef uart_handle;

volatile state_t state = OFF;
volatile temperature_t temperature = LUKEWARM;
uint8_t previous_button_press_time = 0;
...
void init_water_saving_timer() {
	...
	water_saving_timer_handle.Init.Prescaler = 10800 - 1;
	water_saving_timer_handle.Init.Period = 50000 - 1;
...
void init_pwm() {
	...
	red_led_pwm_handle.Instance = TIM2;
	red_led_pwm_handle.Init.Prescaler = 108 - 1;
	red_led_pwm_handle.Init.Period = 100 - 1;
	red_led_pwm_handle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
	red_led_pwm_handle.Init.CounterMode = TIM_COUNTERMODE_UP;

	HAL_TIM_PWM_Init(&red_led_pwm_handle);

	__HAL_RCC_TIM3_CLK_ENABLE()
	;

	blue_led_pwm_handle.Instance = TIM3;
	blue_led_pwm_handle.Init.Prescaler = 108 - 1;
	blue_led_pwm_handle.Init.Period = 100 - 1;
	blue_led_pwm_handle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
	blue_led_pwm_handle.Init.CounterMode = TIM_COUNTERMODE_UP;

	HAL_TIM_PWM_Init(&blue_led_pwm_handle);
	...
	HAL_TIM_PWM_ConfigChannel(&red_led_pwm_handle, &sConfig, TIM_CHANNEL_1);
	HAL_TIM_PWM_ConfigChannel(&blue_led_pwm_handle, sConfig, TIM_CHANNEL_1);

	/* starting PWM */
	HAL_TIM_PWM_Start(&red_led_pwm_handle, TIM_CHANNEL_1);
	HAL_TIM_PWM_START(&blue_led_pwm_handle, TIM_CHANNEL_1);
}
...
int main(void) {
	...
	while (1) {
		...
		__HAL_TIM_SET_COMPARE(&red_led_pwm_handle, TIM_CHANNEL_1,
				red_led_intensity);
		__HAL_TIM_SET_COMPARE(&blue_led_pwm_handle, TIM_CHANNEL_1,
				blue_led_intensity);
	}
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	...
	} else if (GPIO_Pin == warmer_button_handle.Pin && state == ON) {
		if (temperature < HOT) {
			temperature++;
		}
	} else if (GPIO_Pin == colder_button_handle.Pin && state == ON) {
		if (temperature > COLD) {
			temperature--;
		}
	}
